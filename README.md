...
3. Measures you walking from the sensor to the far wall for 60 s, or lets you
   skip to results after the empty-room measurement.
4. Measures you sitting still for 60 s.
5. Proposes editable move and still thresholds per gate, a max distance gate
   from your room length, and a timeout. The margin added above the empty-room
   peak defaults to 10 and can be changed on the results page.
...
- ESPHome 2026.8 or newer, because that release changed the web API entity ids
  to the entity-name form that this tool relies on.
...
- The 18 `gX` move-energy and still-energy sensors need `filters: []`.
  ESPHome's default `throttle_with_priority: 1000ms` emits about one update per
  second while the radar produces 10 or more frames per second, leaving
  empty-room peaks under-sampled. If the device publishes to MQTT,
  `state_topic: null` on those sensors keeps the extra traffic off the broker.
